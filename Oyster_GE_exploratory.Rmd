...
## Colleen adding in Gene Ontology for DESeq {.tabset}

**ANGELA:** You will want to edit the below code to fit your data.

```{r 32 cont v 3hr GO input, include = FALSE}

## create GO input dataframe
GO_DEGs <- data.frame(rn = row.names(res_stn_pco2), res_stn_pco2) %>% # make results a dataframe with rownames as column
  mutate(mutated_p = -log(pvalue)) %>% # take neg log of the p values
  mutate(mutated_p_updown = ifelse(log2FoldChange < 0, mutated_p * -1, mutated_p * 1)) %>% # change sign of data
  dplyr::select(rn, mutated_p_updown) %>% # select gene and modified p value columns only
  na.omit()

colnames(GO_DEGs) <- NULL # remove the column names

write.csv(GO_DEGs, "GO_MWU/GO_files/DEG_GOinput.csv", row.names = FALSE, quote = FALSE) # save output
...
goDatabase = "GO_MWU/GO_scripts/go.obo" # download from http://www.geneontology.org/GO.downloads.ontology.shtml
...
for(d in divisions) {
  ...
  for (f in sig_files_plot){
    ...
    # setting conditions of colours so each experiment has own theme
   # if(grepl(str_name[2], pattern = "32C")) {cols <- c("#969696","#6a51a3","#d9d9d9", "#d8daeb")} else {cols <- c("#969696","#e08214","#d9d9d9","#fee0b6")}
