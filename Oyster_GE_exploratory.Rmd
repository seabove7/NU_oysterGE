...
pca_all_plot <- ggplot(data = ge_plast_all, aes(x = treat, y = dist, colour = treat)) + 
  geom_point(position = position_dodge(width = 0.5), color = "lightpink1") + 
...
Assessment of plasticity (A-D) calculated as the average of the distances of each sample to the mean eigenvalue. N_400 was omitted from this analysis due to (insert reason). A-B depicts box plots of plasticity and each individual sample as a point, while C-D shows the mean and standard error.

## Results
Infected oyster samples in the 2800 pCO2 treatment exhibited the highest plasticity (Figure __), but results from the ANOVA test suggest this was not statistically significant (p = 0.17). Oysters not infected with sponge and in the 2800 treatment had the lowest plasticity (Figure _).
