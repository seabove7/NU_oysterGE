clusteringGOs=function(gen2go,div,cutHeight,isoPath) {
	inname=paste(isoPath, "dissim0_", div, "_", gen2go, sep="")
	outname=paste(isoPath,"cl_", "dissim0_", div,"_", gen2go, sep="")
...
#---------------
gomwuStats=function(input, goDatabase, goAnnotations, goDivision, iso_path, trait_path, Module=FALSE, Alternative="t", adjust.multcomp="BH", clusterCutHeight=0.25, largest=0.1, smallest=5, shuffle.reps=10){
	...
	system(paste("perl ", iso_path, "gomwu_a.pl", " ", goDatabase, " ", goAnnotations, " ", input, " ", goDivision, " ", iso_path, " ", trait_path, " ", extraOptions, sep = ""))
	clusteringGOs(gen2go = goAnnotations, div = goDivision, cutHeight = clusterCutHeight, isoPath = iso_path)
	system(paste("perl ", iso_path, "gomwu_b.pl", " ", goAnnotations, " ", input, " ", goDivision, " ", iso_path, " ", trait_path, sep = ""))

	inname=paste(goDivision, "_", input, sep="")	
	rsq=read.table(paste(trait_path, goDivision, "_", input, sep=""),sep="\t",header=T)
	...
	rr$p.adj=fdr
	fname=paste(trait_path, "MWU_", inname, sep="")
...
#-------------------------
gomwuPlot=function(inFile = input, goAnnotations, goDivision, iso_path, trait_path, level1=0.1, level2=0.05, level3=0.01, absValue=-log(0.05,10),  txtsize = 1, adjusted=TRUE, ze=1, font.family="sans", treeHeight=0.5, colors=NULL) {
	require(ape)
	
  inFile_noExt = gsub("_sigGO.tsv", "", inFile)
  in.raw=paste(trait_path, inFile_noExt, ".csv", sep = "")
	in.mwu=paste(GO_path, inFile, sep = "")
	in.dissim=paste(iso_path, paste("dissim", goDivision, goAnnotations, sep="_"), sep = "")
	
	cutoff=-log(level1,10)
	pv=read.table(in.mwu, sep=",", header=T)
	...
	row.names(diss)=names(diss)
	diss.goods=diss[factor(goods.names),factor(goods.names)]
